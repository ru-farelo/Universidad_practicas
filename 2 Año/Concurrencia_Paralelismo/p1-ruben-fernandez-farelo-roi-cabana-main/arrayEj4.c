...
void *increment(void *thread_arg)
{
    ...
    for(int i = 0; i < data->iterations; i++) {
        //Comprueba si nos hemos quedado sin iteraciones
        pthread_mutex_lock(data->iteration_mutex);
        if(*data->iteration_left==0){
            pthread_mutex_unlock(data->iteration_mutex);
            return 0;
        }
        *data->iteration_left=*data->iteration_left-1;
        pthread_mutex_unlock(data->iteration_mutex);
...
int main (int argc, char **argv)
{
    ...
    int                 *iterations_left_inc;
    int                 *iterations_left_trans;
    pthread_mutex_t     control_inc,control_trans;
    ...
    //reservamos memoria para las variables decontrol
    iterations_left_inc= malloc(sizeof (int));
    iterations_left_trans= malloc(sizeof (int));

    //Inicializamos los mutex de control
    pthread_mutex_init(&control_inc, NULL);
    pthread_mutex_init(&control_trans, NULL);

    //Inicializamos variable de conteo de iteraciones
    *iterations_left_inc=opt.iterations;
    *iterations_left_trans=opt.iterations;
    ...
    for (int i = 0; i < opt.num_threads; i++) {//Creamos tantos hilos como iteraciones haya
        ...
        arg_list[i]=(struct thread_data){i, opt.iterations, opt.delay, &arr,
                                         mutex_list,iterations_left_inc,
                                         &control_inc};
    ...
    pthread_mutex_destroy(&control_inc); //Destruimos los mutex
    pthread_mutex_destroy(&control_trans); //Destruimos los mutex
    //hacemos los frees correspondientes
    free(threads);
    free(ids);
    free(mutex_list);
    free(iterations_left_inc);
    free(iterations_left_trans);
    free(arg_list);
